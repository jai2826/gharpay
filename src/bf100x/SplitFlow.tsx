...
// GHARPAY_TODO: Added Suggestnext Function
function suggestNext(stepTitle: string, waitingOn: string): string {
  const t = stepTitle.toLowerCase();
  if (/check.?in/.test(t)) return "Prepare check-in";
  if (/pay|booking amount|token/.test(t)) return "Chase payment";
  if (/approv/.test(t)) return "Chase property approval";
  if (/quot/.test(t)) return "Send quotation";
  if (/decision|feedback|post-tour|say after/.test(t)) return "Follow up on decision";
  if (/confirm/.test(t) && /tour/.test(t)) return "Confirm the tour";
  if (/tour|visit/.test(t)) return "Schedule the tour";
  if (/propert|option|match|shortlist/.test(t)) return "Share property options";
  if (/customer/i.test(waitingOn)) return "Send WhatsApp message";
  return "Call the customer";
}
// GHARPAY_TODO: Added toLocalInput and inHours functions to handle local date-time formatting and default due date calculation
const toLocalInput = (d: Date) =>
  new Date(d.getTime() - d.getTimezoneOffset() * 60_000).toISOString().slice(0, 16);
const inHours = (n: number) => toLocalInput(new Date(Date.now() + n * 3_600_000));
...
export function SplitFlow({
  ...
  // GHARPAY_TODO: Added due date for next action, default 2 hours from now
  const [due, setDue] = useState(() => inHours(2));
  ...
  useEffect(() => {
    ...
    const match = want
      ? leads.find(
          (l) => (l.canonicalId || canonicalCustomerId({ phone: l.phone, name: l.name })) === want,
        )
      : undefined;
    if (!match) {
      setLeadId("");
  ...
  const screen =
    SCREENS.find((s) => s.id === screenId) ?? (lead ? currentScreen(lead.f ?? {}) : SCREENS[0]!);
  const idx = screenIndex(screen.id);
  const h = mounted && lead ? health(lead) : undefined;

  // GHARPAY_TODO: If the lead already has a next action and a due date, use that. Otherwise, suggest one based on the current step and waitingOn, and set a default due date 2 hours from now.
  useEffect(() => {
    if (!lead) return;
    if (lead.nextAction && lead.nextActionAt) {
      setNextAction(lead.nextAction);
      setDue(toLocalInput(new Date(lead.nextActionAt)));
    } else {
      setNextAction(suggestNext(h?.step?.title ?? "", h?.waitingOn ?? ""));
      setDue(inHours(2));
    }
  }, [lead?.id, lead?.nextAction, lead?.nextActionAt, h?.step?.key]);
  FileList;
  ...
  const stats = useMemo(() => {
    ...
    const left = leads.filter(
      (l) => l.stage !== "CLOSED" && (!l.owner || !l.nextAction || !l.nextActionAt),
    ).length;
  ...
  // GHARPAY_TODO: If the lead has a next action and a due date, lock them in. Otherwise, set them to the current values of nextAction and due.
  function lock() {
    if (!lead) return;
    setNext(lead.id, nextAction, new Date(due).toISOString());
    toast.success("Next step and deadline locked");
  }

  function nextCustomer() {
    // never leave a customer without a next step and a deadline
    if (lead && (!lead.nextAction || !lead.nextActionAt)) {
      setNext(lead.id, nextAction, new Date(due).toISOString());
      toast.success(`Next step locked for ${lead.name}: ${nextAction}`);
    }
  ...
  return (
    <div
      className={cn(
        ...
        panelOnly ? "h-full" : embedded ? "h-[calc(100vh-10rem)]" : "h-screen",
      )}
    >
      <div
        className="flex min-w-0 flex-col overflow-hidden bg-background"
        style={{ width: panelOnly ? "100%" : `${widthPct}%` }}
      >
            ...
            <div className="flex min-w-0 items-center gap-1 overflow-hidden">
              ...
              {mounted && (
                <div className="flex min-w-0 gap-1 overflow-x-auto">
                  ...
                  {stats.late > 0 && (
                    <Badge variant="destructive" className="shrink-0 px-1 text-[9px]">
                      {stats.late} late
                    </Badge>
                  )}
                </div>
                  ...
                  {WIDTH_PRESETS.map((p) => (
                    <button
                      ...
                      className={cn(
                        ...
                        widthPct === p ? "bg-primary/15 text-primary" : "text-muted-foreground",
                      )}
                    >
                      {p}%
                    </button>
          ...
          <div className="shrink-0 border-b px-2 py-1">
            ...
            {mounted && h && (
              <div className="mt-1 flex flex-wrap gap-1 text-[10px]">
                ...
                <Badge variant={lead.owner ? "secondary" : "destructive"} className="text-[10px]">
                  {lead.owner ?? "no owner"}
                </Badge>
                <Badge variant="outline" className="text-[10px]">
                ...
                <Badge
                  variant={lead.nextActionAt && h.sla !== "LATE" ? "outline" : "destructive"}
                  className="text-[10px]"
                >
                  {lead.nextActionAt
                    ? h.sla === "LATE"
          ...
          {pane === "CLOSING" ? (
            ...
            <div className="space-y-2">
              <p className="text-[10px] text-muted-foreground">
                Four drafts a day for {me} — D1, D2, D3, D4 · {BATCH_SIZE} customers each. Close a
                draft when all 30 have a next step and a deadline.
              </p>
              {ROUNDS.map((r) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      batch?.closedAt && "border-primary/40 bg-primary/5",
                    )}
                  >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-[10px] text-muted-foreground">
                          {!batch
                            ? "Not opened yet"
                            : batch.closedAt
                              ? `Closed ${new Date(batch.closedAt).toLocaleString()}${batch.closeNote ? ` — ${batch.closeNote}` : ""}`
                        ...
                        {rows.slice(0, 30).map((l) => (
                          <button
                            ...
                            className={cn(
                              ...
                              l.id === lead?.id && "border-primary bg-primary/5",
                            )}
                          >
                            <span className="truncate text-[11px]">{l.name}</span>
                            <Badge
                              variant={l.nextAction && l.nextActionAt ? "outline" : "destructive"}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      >
                        <p className="truncate text-xs font-medium">
                          {l.name}{" "}
                          <span className="font-normal text-muted-foreground">{l.phone}</span>
                        </p>
                        <p className="truncate text-[10px] text-muted-foreground">
                          {lh.stepNo}. {lh.step?.title ?? "Checked in"} · {l.owner ?? "no owner"} ·{" "}
            ...
            <p className="pt-10 text-center text-sm text-muted-foreground">
              Nothing left in the queue — every customer is closed or checked in.
            </p>
          ) : pane === "WORK" ? (
            <ScreenPanel
              lead={lead}
        ...
        {lead && (
          <footer className="shrink-0 border-t px-2 py-1">
            {/* GHARPAY_TODO: Display a summary of the current customer's status */}
            <p className="pb-1 text-[10px] font-medium text-primary">
              Result: more tours and bookings actually happen — every customer leaves with an owner,
              a next step and a deadline.
            </p>
            ...
            <div className="flex items-center gap-1 overflow-x-auto">
              ...
              {!lead.owner && (
                <Button
                  ...
                  onClick={() => {
                    claim(lead.id);
                    toast.success(`${lead.name} is yours, ${me}`);
                  }}
                >
              ...
              <Button
                ...
                className="h-7 px-2 text-[10px]"
                onClick={() => {
                  escalate(lead.id, "Operator asked for help");
              ...
              {/* GHARPAY_TODO: Updated the date picker for the due date */}
              <Input
                type="datetime-local"
        ...
        <Dialog open={!!closingId} onOpenChange={(o) => !o && setClosingId(null)}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Close this draft</DialogTitle>
            </DialogHeader>
            <label className="block text-xs font-medium">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  if (closingId) closeBatch(closingId, closeNote);
                  setClosingId(null);
        ...
        <>
          ...
          <div className="flex min-w-0 flex-1 items-center justify-center bg-muted/30 p-4 text-center">
            <p className="text-[11px] text-muted-foreground">
              ...
              <br />
              Drag the grey bar, or use the width buttons, to set the sizes you want.
            </p>
          </div>
        </>
      )}
